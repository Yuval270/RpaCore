...
import me.yuval270.rpacore.imanagers.DamageManagers;
import me.yuval270.rpacore.imanagers.Manager;
import me.yuval270.rpacore.RpaCore;
import me.yuval270.rpacore.imanagers.QuitJoinManagers;
...
public class ResetCharacterManager implements Manager, DamageManagers, QuitJoinManagers {
    ...
    private final List<String> commandsBeforeNickname = new ArrayList<>();
    ...
    public ResetCharacterManager() {
        ...
        commandsBeforeNickname.addAll(configSection.getStringList("commands_before_nick_name"));
    ...
    public void executeBeforeNickCommands(String playerName) {
        commandsBeforeNickname.forEach(command ->
    ...
    public boolean handleQuit(PlayerQuitEvent event) {
        ...
        if (stage != Stage.CHOOSING_NICKNAME)
            main.getResetCharacterTable().updateField(uuid.toString(), stage.getOnQuit().name());
        activePlayers.remove(uuid);
        return false;
    }
